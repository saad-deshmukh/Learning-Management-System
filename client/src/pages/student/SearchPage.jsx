...
const SearchPage = () => {
  ...
  // make query safe (never null)
  const query = searchParams.get("query") || "";

  const [selectedCategories, setSelectedCategories] = useState([]);
  const [sortByPrice, setSortByPrice] = useState("");

  //  restore filters from URL (refresh / back / forward)
  useEffect(() => {
    const urlCategories = searchParams.get("categories");
    const urlSort = searchParams.get("sortByPrice");
  ...
  //preserve query + sync filters to URL
...
const CourseSkeleton = () => (
  <div className="flex flex-col md:flex-row gap-6 border-b border-stone-200 dark:border-stone-800 pb-6">
    ...
    <div className="flex-1 space-y-3">
      ...
      <Skeleton className="h-5 w-16 bg-stone-200 dark:bg-stone-800" />
    </div>
  </div>
);
